<template>
    <div class="flex">
      <Carousel :wrap-around="true" ref="productsCarousel">
        <Slide v-for="(product, index) in beforeAndAfterList" :key="index">
            <div
  ...
  import antesEDepois from "@/plugins/beforaandafter";
  import { VaImage, VaAvatar } from "vuestic-ui/web-components";
  
  import "vue3-carousel/dist/carousel.css";
  
  export default defineComponent({
    name: "BeforeAndAfterComponent",
    setup() {
      const beforeAndAfterList = ref([]);
  
      onMounted(() => {
        beforeAndAfterList.value = antesEDepois;
      });
